...
[快速开始](#quick-start) · [抓取方式](#capture-methods) · [切图导出](#slice-export) · [Codex 插件](#codex-plugin) · [UI 还原](#ui-restore)
...
> 💡 `start.sh` 会自动检查并准备项目依赖，不需要再手动执行其他启动命令。

<a id="capture-methods"></a>
...
脚本会自动构建前端、生成独立 MCP，并打包 Tauri 正式版。产物位于：
...
- 在插件列表中看到 `DesignBridge`。
- 在 **设置 → MCP 服务器** 中看到 `designbridge`。
...
> 🔍 切图预览可以点击放大。空白、完全透明和 `1 × 1 px` 的无效图片会被自动过滤。
